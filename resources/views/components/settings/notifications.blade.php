...
<form action="{{ route('settings.update') }}" method="POST" class="space-y-6">
    ...
    <div>
        <h3 class="text-base font-bold text-gray-900 mb-1">{{ __('Notification Settings') }}</h3>
        <p class="text-xs text-gray-500">{{ __('Choose what updates you want to receive.') }}</p>
    </div>
    ...
    <div class="space-y-4 border-t border-gray-100 pt-4">
        <label class="flex items-start gap-3 cursor-pointer group">
            ...
            <div>
                <span class="block text-sm font-medium text-gray-700 group-hover:text-gray-900 transition">{{ __('Email Digests') }}</span>
                <span class="block text-xs text-gray-400">{{ __('Receive weekly summaries of your activity and notes.') }}</span>
            </div>
        </label>
        ...
        <label class="flex items-start gap-3 cursor-pointer group">
            ...
            <div>
                <span class="block text-sm font-medium text-gray-700 group-hover:text-gray-900 transition">{{ __('Browser Push Notifications') }}</span>
                <span class="block text-xs text-gray-400">{{ __('Get instant alerts on actions inside shared folders.') }}</span>
            </div>
        </label>
    </div>

    <div class="pt-6 border-t border-gray-100">
        <h3 class="text-base font-bold text-gray-900 mb-1">{{ __('Delivery Channels') }}</h3>
        <p class="text-xs text-gray-500 mb-4">{{ __('Where should we deliver your instant alerts?') }}</p>
