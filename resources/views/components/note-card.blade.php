...
<div class="relative group p-5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm hover:shadow-md hover:border-indigo-100 dark:hover:border-indigo-900 transition duration-200 flex flex-col justify-between min-h-[190px]">

    <div class="mb-4">
        <div class="pr-20">
            <a href="{{ route('notes.show', $note->id) }}" class="block group/title">
                <h4 class="font-bold text-gray-900 dark:text-gray-100 group-hover/title:text-indigo-600 dark:group-hover/title:text-indigo-400 transition line-clamp-1 tracking-tight text-base">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-3 line-clamp-3 leading-relaxed">
    ...
    <div class="space-y-3 pt-3 border-t border-gray-50 dark:border-gray-700/50 text-xs text-gray-400 dark:text-gray-500">
        <!-- ОТОБРАЖЕНИЕ ТЕГОВ -->
        @if($note->tags && $note->tags->isNotEmpty())
            <div class="flex flex-wrap gap-1.5 max-h-[48px] overflow-hidden">
                @foreach($note->tags as $tag)
                    <a href="{{ route('tags.show', $tag->id) }}" class="inline-flex items-center px-2 py-0.5 bg-gray-50 dark:bg-gray-700/50 border border-gray-150 dark:border-gray-700 rounded-full font-medium text-gray-500 dark:text-gray-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-indigo-800 transition text-[11px]">
                        <span class="text-indigo-400 dark:text-indigo-500 font-bold mr-0.5">#</span>{{ $tag->name }}
                    </a>
                @endforeach
            </div>
        @endif
        ...
        <div class="flex flex-wrap items-center gap-x-3 gap-y-1.5 font-medium">
            @if($note->folders && $note->folders->isNotEmpty())
                @foreach($note->folders as $folder)
                    <a href="{{ route('folders.show', $folder->id) }}" class="flex items-center gap-1.5 hover:text-indigo-600 dark:hover:text-indigo-400 transition text-gray-500 dark:text-gray-400">
                        <svg class="w-3.5 h-3.5 text-amber-500 dark:text-amber-400 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                @endforeach
            @else
                <span class="flex items-center gap-1.5 font-normal italic text-gray-300 dark:text-gray-600">
                    <svg class="w-3.5 h-3.5 text-gray-200 dark:text-gray-700 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/>
                    </svg>
                    {{ __('Without folder') }}
                </span>
    ...
    <div class="absolute top-4 right-4 flex items-center gap-1.5 z-10">
        <form action="{{ route('notes.pin', $note->id) }}" method="POST">
            @csrf
            @method('PATCH')
            <button type="submit" class="p-2 border rounded-xl shadow-sm transition duration-200 opacity-0 group-hover:opacity-100 {{ $note->is_pinned ? 'bg-indigo-50 dark:bg-indigo-950/40 border-indigo-200 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-950/80 hover:text-indigo-700 dark:hover:text-indigo-300' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-indigo-800' }}">
                <svg class="w-4 h-4 transition duration-200 {{ $note->is_pinned ? 'fill-indigo-600 dark:fill-indigo-400 text-indigo-600 dark:text-indigo-400 rotate-45' : 'fill-none text-gray-400 dark:text-gray-500 group-hover:text-indigo-600 dark:group-hover:text-indigo-400' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <button type="button" class="js-btn-preview p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-400 dark:text-gray-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-indigo-800 shadow-sm transition duration-200 opacity-0 group-hover:opacity-100"
    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:p-0 relative">
        ...
        <div class="relative inline-block align-middle bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full border border-gray-100 dark:border-gray-700 z-10">
            <div class="bg-white dark:bg-gray-800 px-6 pt-6 pb-5">
                ...
                <div class="text-sm text-gray-600 dark:text-gray-300 border-t border-gray-100 dark:border-gray-700 pt-4 max-h-[50vh] overflow-y-auto whitespace-pre-wrap leading-relaxed prose prose-indigo dark:prose-invert max-w-none" id="modalContent">
                    {{ __('Note content...') }}
                </div>
            </div>
