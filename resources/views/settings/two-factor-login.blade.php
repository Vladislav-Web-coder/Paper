    ...
    <div class="max-w-md mx-auto py-6">
        <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 sm:p-8 space-y-6">
            ...
            <div class="text-center space-y-2">
                ...
                <h1 class="text-xl font-bold text-gray-900 tracking-tight">{{ __('Security Check Required') }}</h1>
                <p class="text-xs text-gray-500 max-w-xs mx-auto leading-relaxed">{{ __('Please enter the 6-digit authentication code from your app, or use an emergency 8-character recovery code.') }}</p>
            </div>
            ...
            <form action="{{ route('2fa.verify') }}" method="POST" class="space-y-5">
                ...
                <div>
                    <label for="code" class="block text-xs font-medium text-gray-700 mb-1.5">{{ __('Authentication / Recovery Code') }}</label>
                    <input type="text"
                           id="code"
                           name="code"
                           required
                           ...
                           class="w-full px-4 py-2.5 rounded-xl border @error('code') border-rose-300 focus:ring-rose-500 focus:border-rose-500 @else border-gray-200 focus:ring-indigo-500 focus:border-indigo-500 @enderror bg-gray-50/50 text-center font-medium text-sm focus:outline-none focus:ring-2 transition placeholder:text-gray-400 uppercase"
                    >
                    @error('code')
            ...
            <div class="border-t border-gray-100 pt-4 text-center">
                <form action="{{ route('logout') }}" method="POST" class="inline-block">
                    @csrf
                    <button type="submit" class="text-xs text-gray-400 hover:text-gray-600 font-medium transition underline underline-offset-4 decoration-dashed">
